...
class FewShotDataloader(object):
    ...
    def get_iterator(self, epoch=0):
        ...
        def load_function(iter_idx):
            ...
            Xt, Yt = self.dataset.createExamplesTensorData(query_e)
            if len(shot_e) > 0:
                Xe, Ye = self.dataset.createExamplesTensorData(shot_e)
                return Xe, Ye, Xt, Yt
            else:
                return Xt, Yt
...
def plot_images(data, n_rows, name='image.png'):
    num = len(data)
    n_cols = int(np.ceil(num/n_rows))
    # x_shot = data.reshape(n_rows,n_cols,3,84,84)
   
    fig, ax = plt.subplots(1, figsize=(10,10))
    grid_img = torchvision.utils.make_grid(data, nrow=n_rows)
...
class FewShotDataloaderSample(object):
    def __init__(self, dataset, kway=5, kshot=1, kquery=1, batch_size=1, num_workers=2,
                 epoch_size=2000, shuffle=True, fixed=False):
    ...
    def get_iterator(self, epoch=0, n=5,k=5):
        random.seed(epoch)
        np.random.seed(epoch)
        # torch.manual_seed(epoch)
        # torch.cuda.manual_seed_all(epoch)
        ...
        def load_function(iter_idx):
            num_samples = self.kshot + self.kquery
            categories = self.sampleCategories(n)
            shot_e, query_e = self.sample_examples_for_categories(categories, k, num_samples-k)
            Xt, Yt = self.dataset.createExamplesTensorData(query_e)
            if len(shot_e) > 0:
                Xe, Ye = self.dataset.createExamplesTensorData(shot_e)
                return Xe, Ye, Xt, Yt
    ...
    def __call__(self, epoch=0, n=5,k=5):
        return self.get_iterator(epoch,n,k)
