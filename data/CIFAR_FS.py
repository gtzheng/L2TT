...
from metadatas.CIFAR_FS import Cutout
...
class CIFAR_FS(data.Dataset):
    def __init__(self, phase='train', mean_pix=[x/255.0 for x in [129.37731888, 124.10583864, 112.47758569]], std_pix = [x/255.0 for x in [68.20947949, 65.43124043, 70.45866994]], do_not_use_random_transf=False, aug_num=1, aug_method='normal'):
    ...
    def __getitem__(self, index):
        if self.aug_method == 'auto_augment' and self.aug_num > 1:
            img, label = self.data[index], self.labels[index]
            patch_list=[]
            for _ in range(self.aug_num):
                patch_list.append(self.transform(Image.fromarray(img)))
