...
def MetaOptNetHead_Ridge(query, support, support_labels, n_way, n_shot, lambda_reg=50.0, double_precision=False):
    ...
    kernel_matrix += lambda_reg * torch.eye(n_support).expand(tasks_per_batch, n_support, n_support).to(query.device)
    ...
    dummy = Variable(torch.Tensor()).to(query.device)      # We want to ignore the equality constraint.

    if double_precision:
        G, e, C, h = [x.double().to(query.device) for x in [G, e, C, h]]

    else:
        G, e, C, h = [x.float().to(query.device) for x in [G, e, C, h]]
...
def MetaOptNetHead_SVM_He(query, support, support_labels, n_way, n_shot, C_reg=0.01, double_precision=False):
    ...
    V = (support_labels * n_way - torch.ones(tasks_per_batch, n_support, n_way).to(query.device)) / (n_way - 1)
    ...
    if double_precision:
        ...
        G, e, C, h = [x.to(query.device) for x in [G, e, C, h]]
...
def MetaOptNetHead_SVM_CS(query, support, support_labels, n_way, n_shot, C_reg=0.1, double_precision=False, maxIter=15):
    ...
    block_kernel_matrix += 1.0 * torch.eye(n_way*n_support).expand(tasks_per_batch, n_way*n_support, n_way*n_support).to(query.device)
    ...
    #print (A.size(), b.size())
    if double_precision:
        G, e, C, h, A, b = [x.double().to(query.device) for x in [G, e, C, h, A, b]]
    else:
        G, e, C, h, A, b = [x.float().to(query.device) for x in [G, e, C, h, A, b]]
...
def MetaOptNetHead_SVM_CS_MIX(query, support, support_labels, n_way, n_shot, C_reg=0.1, double_precision=False, maxIter=15):
    """
    Fits the support set with multi-class SVM and 
    returns the classification score on the query set.
    ...
    support_labels_a_one_hot = one_hot(support_labels_a.view(tasks_per_batch * n_support), n_way)  # (tasks_per_batch * n_support, n_support)
    support_labels_a_one_hot = support_labels_a_one_hot.view(tasks_per_batch, n_support, n_way)
    support_labels_b_one_hot = one_hot(support_labels_b.view(tasks_per_batch * n_support), n_way)  # (tasks_per_batch * n_support, n_support)
    support_labels_b_one_hot = support_labels_b_one_hot.view(tasks_per_batch, n_support, n_way)
    lam = torch.tensor(lam).to(query.device)
    support_labels_one_hot = lam.view(-1, 1, 1) * support_labels_a_one_hot + (1 - lam).view(-1, 1, 1) * support_labels_b_one_hot
...
def MetaOptNetHead_SVM_WW(query, support, support_labels, n_way, n_shot, C_reg=0.00001, double_precision=False):
    ...
    kernel_matrix = computeGramMatrix(support, support) + torch.ones(tasks_per_batch, n_support, n_support).to(query.device)
    ...
    kernel_matrix_mask_second_term = kernel_matrix_mask_second_term == torch.arange(n_way).long().repeat(n_support).reshape(n_support, n_way).transpose(1, 0).reshape(1, -1).repeat(n_support, 1).to(query.device)
    ...
    C_mat = C_reg * torch.ones(tasks_per_batch, n_way * n_support).to(query.device) - C_reg * Y_support

    C = Variable(torch.cat((id_matrix, -id_matrix), 1))
    #C = Variable(torch.cat((id_matrix_masked, -id_matrix_masked), 1))
    zer = torch.zeros(tasks_per_batch, n_way * n_support).to(query.device)
    ...
    # Compute the classification score.
    compatibility = computeGramMatrix(support, query) + torch.ones(tasks_per_batch, n_support, n_query).to(query.device)
    compatibility = compatibility.float()
...
class ClassificationHead_Mixup(nn.Module):
    def __init__(self, base_learner='MetaOptNet', enable_scale=True):
        super(ClassificationHead_Mixup, self).__init__()
        self.base_learner = base_learner
        if ('SVM-CS' in base_learner):
            self.head = MetaOptNetHead_SVM_CS_MIX
        elif ('Ridge' in base_learner):
            self.head = MetaOptNetHead_Ridge
        elif ('Proto' in base_learner):
            self.head = ProtoNetHead
        ...
        # Add a learnable scale
        self.enable_scale = enable_scale
        self.scale = nn.Parameter(torch.FloatTensor([1.0])) if self.enable_scale else None

    def forward(self, query, support, support_labels_a, support_labels_b, lam, n_way, n_shot, **kwargs):
        if self.enable_scale:
            lq = self.scale * self.head(query, support, support_labels_a, support_labels_b, lam, n_way, n_shot, **kwargs)
            return lq 
        else:
            return self.head(query, support, support_labels_a, support_labels_b, lam, n_way, n_shot, **kwargs)


def R2D2_Woodbury_Mixup(query, support, support_labels_a, support_labels_b, lam, n_way, n_shot, l2_regularizer_lambda=50.0):
    """
    Fits the support set with ridge regression and
    returns the classification score on the query set.
    ...
    support_labels_a_one_hot = one_hot(support_labels_a.reshape(tasks_per_batch * n_support), n_way)
    support_labels_a_one_hot = support_labels_a_one_hot.reshape(tasks_per_batch, n_support, n_way)
    support_labels_b_one_hot = one_hot(support_labels_b.reshape(tasks_per_batch * n_support), n_way)
    support_labels_b_one_hot = support_labels_b_one_hot.reshape(tasks_per_batch, n_support, n_way)
    lam = torch.tensor(lam).float().to(query.device)
    support_labels_one_hot = lam.view(-1, 1, 1) * support_labels_a_one_hot + (1 - lam).view(-1, 1, 1) * support_labels_b_one_hot
    ...
    # Compute the classification score.
    # score = W X
    logits_q = torch.bmm(query, ridge_sol)

    return logits_q


class R2D2Head_Mixup(nn.Module):
    def __init__(self, affine=True):
        super(R2D2Head_Mixup, self).__init__()
    ...
    def forward(self, query, support, support_labels_a, support_labels_b, lam, n_way, n_shot, **kwargs):
        if self.affine:
            #return self.scale * R2D2_Woodbury_Mixup(query, support, support_labels_a, support_labels_b, lam, n_way, n_shot, **kwargs) + self.bias
            lq = self.scale * R2D2_Woodbury_Mixup(query, support, support_labels_a, support_labels_b, lam, n_way, n_shot, **kwargs) + self.bias
            return lq
        else:
            return R2D2_Woodbury_Mixup(query, support, support_labels_a, support_labels_b, lam, n_way, n_shot, **kwargs)
